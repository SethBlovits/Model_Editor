#ifndef SLUGS_IMGUI_H
#define SLUGS_IMGUI_H

   
#include "cimgui.h"
#include "mathUtil.h"
#include "imgui_hlsl.h"
//so we need some sort of backend for this and include the dx12 shader for rendering the window

extern const unsigned char imgui_ps_cso[];
extern const unsigned int imgui_ps_cso_size;

extern const unsigned char imgui_vs_cso[];
extern const unsigned int imgui_vs_cso_size;

//DECLARE THE GLOBAL STATE THINGS WE NEED    
slg_pipeline slimgui_pip;
struct slugs_imgui_frame_buffer {
    slg_buffer index_buffers[FrameCount];
    slg_buffer vertex_buffers[FrameCount];
    slg_bindings bind[FrameCount];//SCUFFED?? Could be
}slimgui_frame_buffer;
//This is a bit of a nasty solution that doesn't scale well but it's good for now
typedef struct{
    int mouse_x;
    int mouse_y;

    bool left_mouse_down;
    bool right_mouse_down;

    float mouse_scroll_delta;

    bool key_a_down;
    bool key_b_down;
    bool key_c_down;
    bool key_d_down;
    bool key_e_down;
    bool key_f_down;
    bool key_g_down;
    bool key_h_down;
    bool key_i_down;
    bool key_j_down;
    bool key_k_down;
    bool key_l_down;
    bool key_m_down;
    bool key_n_down;
    bool key_o_down;
    bool key_p_down;
    bool key_q_down;
    bool key_r_down;
    bool key_s_down;
    bool key_t_down;
    bool key_u_down;
    bool key_v_down;
    bool key_w_down;
    bool key_x_down;
    bool key_y_down;
    bool key_z_down;

    bool key_0_down;
    bool key_1_down;
    bool key_2_down;
    bool key_3_down;
    bool key_4_down;
    bool key_5_down;
    bool key_6_down;
    bool key_7_down;
    bool key_8_down;
    bool key_9_down;

    // Modifier keys
    bool key_shift_down;
    bool key_ctrl_down;
    bool key_alt_down;
    bool key_caps_lock_down;

    // Navigation keys
    bool key_tab_down;
    bool key_enter_down;
    bool key_escape_down;
    bool key_backspace_down;
    bool key_space_down;

    // Arrow keys
    bool key_left_down;
    bool key_right_down;
    bool key_up_down;
    bool key_down_down;

    // Editing / navigation block
    bool key_insert_down;
    bool key_delete_down;
    bool key_home_down;
    bool key_end_down;
    bool key_page_up_down;
    bool key_page_down_down;

    // Function keys
    bool key_f1_down;
    bool key_f2_down;
    bool key_f3_down;
    bool key_f4_down;
    bool key_f5_down;
    bool key_f6_down;
    bool key_f7_down;
    bool key_f8_down;
    bool key_f9_down;
    bool key_f10_down;
    bool key_f11_down;
    bool key_f12_down;

    // Numpad keys
    bool key_numpad_0_down;
    bool key_numpad_1_down;
    bool key_numpad_2_down;
    bool key_numpad_3_down;
    bool key_numpad_4_down;
    bool key_numpad_5_down;
    bool key_numpad_6_down;
    bool key_numpad_7_down;
    bool key_numpad_8_down;
    bool key_numpad_9_down;

    bool key_numpad_add_down;
    bool key_numpad_subtract_down;
    bool key_numpad_multiply_down;
    bool key_numpad_divide_down;
    bool key_numpad_enter_down;
    bool key_numpad_decimal_down;

    // Lock / system keys
    bool key_num_lock_down;
    bool key_scroll_lock_down;
    bool key_print_screen_down;
    bool key_pause_down;

    // Symbol keys
    bool key_minus_down;
    bool key_equals_down;
    bool key_left_bracket_down;
    bool key_right_bracket_down;
    bool key_backslash_down;
    bool key_semicolon_down;
    bool key_apostrophe_down;
    bool key_comma_down;
    bool key_period_down;
    bool key_slash_down;
    bool key_grave_down; //

    char input_char;
}slimgui_input_state_t;

#ifdef SLUGS_IMGUI_IMPLEMENTATION
unsigned char* pixels = NULL;
slimgui_input_state_t slimgui_input_state;
ImGuiIO* io;

struct{
    slg_render_texture* rt_list;
    int num_rt;
    slg_bindings* rt_binding_list;
}slimgui_rt_register;
slg_buffer rt_index_buffer;
slg_buffer rt_vertex_buffer;
slg_buffer rt_mvp_buffer;
slg_texture font_atlas;

static inline void slimgui_process_char_event(char input_char){
    slimgui_input_state.input_char = input_char;
    ImGuiIO_AddInputCharacter(io,input_char);
}

void slimgui_setup(){
    igCreateContext(NULL);
    
    io = igGetIO();
    io->IniFilename = NULL;
    io->DisplaySize.x = (float)slg_d3d12_state.appdata.width;
    io->DisplaySize.y = (float)slg_d3d12_state.appdata.height;

    int tex_height = 0;
    int tex_width = 0;
    

    ImFontAtlas_AddFontFromFileTTF(io->Fonts, "C:/MaterialEditor/JetBrainsMono-SemiBold.ttf", 17.0f, NULL, NULL);
    ImFontAtlas_GetTexDataAsRGBA32(io->Fonts,&pixels,&tex_width,&tex_height,NULL);

    slimgui_rt_register.num_rt = 5;
    slimgui_rt_register.rt_binding_list = malloc(sizeof(slg_bindings)*5);
    slimgui_rt_register.rt_list = malloc(sizeof(slg_render_texture)*5);
 
    font_atlas = slg_make_texture(&(slg_texture_desc){ 
        .height = tex_height,
        .width = tex_width,
        .pixel_size = 4,
        .tex_type = SLG_TEXTURE_TYPE_2D,
        .texture = pixels
    });
    io->Fonts->TexID = (ImTextureID)font_atlas.buffer;

    ImFontAtlas_ClearTexData(io->Fonts);
    Mat4 viewMat = identityMat4();
    Mat4 projMat = orthographicMat4(0.0f,io->DisplaySize.x,io->DisplaySize.y,0.0f,-1.0f,1.0f);
    
    Mat4 mvpMat4 = mulMat4(projMat,viewMat);

    slg_buffer mvp_buffer = slg_make_buffer(&(slg_buffer_desc){
        .buffer = (void*)&mvpMat4,
        .buffer_size = sizeof(Mat4),
        .buffer_stride = sizeof(Mat4),
        .usage = SLG_BUFFER_USAGE_CONSTANT_BUFFER
    });

    rt_mvp_buffer = slg_make_buffer(&(slg_buffer_desc){
        .buffer = (void*)&mvpMat4,
        .buffer_size = sizeof(Mat4),
        .buffer_stride = sizeof(Mat4),
        .usage = SLG_BUFFER_USAGE_CONSTANT_BUFFER
    });


    ImDrawVert temp_vert_buffer[10000] = {0};
    ImDrawIdx temp_index_buffer[10000] = {0};

    rt_index_buffer = slg_make_buffer(&(slg_buffer_desc){
        .buffer = temp_index_buffer,
        .buffer_size = sizeof(ImDrawIdx)*10000,
        .buffer_stride = sizeof(ImDrawIdx)
    });
    rt_vertex_buffer = slg_make_buffer(&(slg_buffer_desc){
        .buffer = temp_vert_buffer,
        .buffer_size = sizeof(ImDrawVert)*10000,
        .buffer_stride = sizeof(ImDrawVert)
    });

    for(int i = 0;i<FrameCount;i++){
        slimgui_frame_buffer.index_buffers[i] = slg_make_buffer(&(slg_buffer_desc){
            .buffer = temp_index_buffer,
            .buffer_size = sizeof(ImDrawIdx)*10000,
            .buffer_stride = sizeof(ImDrawIdx)
            
        });
        slimgui_frame_buffer.vertex_buffers[i] = slg_make_buffer(&(slg_buffer_desc){
            .buffer = temp_vert_buffer,
            .buffer_size = sizeof(ImDrawVert)*10000,
            .buffer_stride = sizeof(ImDrawVert)
        });
        slimgui_frame_buffer.bind[i] = slg_make_bindings(&(slg_bindings_desc){
        .index_buffer = slimgui_frame_buffer.index_buffers[i],
        .vertex_buffer = slimgui_frame_buffer.vertex_buffers[i],
        .uniforms = IMGUI_HLSL_MAKE_UNIFORMS((IMGUI_HLSL_UNIFORMS){
            .g_texture = font_atlas,
            .MVPConstantBuffer = mvp_buffer
        })     
        });
    }

    slg_shader shd = slg_make_shader(&IMGUI_SHADER_DESC);
    DXGI_FORMAT format_override[] = {DXGI_FORMAT_R32G32_FLOAT, DXGI_FORMAT_R32G32_FLOAT,DXGI_FORMAT_R8G8B8A8_UNORM};
    slimgui_pip = slg_make_pipeline(&(slg_pipeline_desc){
        .shader = shd,
        .depth_stencil_desc = (slg_depth_stencil_desc){
            .depth_enable = false,
            .write_mask = SLG_DEPTH_WRITE_MASK_ALL,
            .compare_func = SLG_COMPARISON_FUNC_ALWAYS,
            .stencil_enable = false
        },
        .rasterizer_desc = (slg_rasterizer_desc){
            .fill_mode = SLG_FILL_MODE_SOLID,
            .cull_mode = SLG_CULL_MODE_NONE,
            .multisample_enable = false,
            .antialiased_line_enable = false,
        },
        .blend_desc = (slg_blend_desc){
            .blend_enable = true,
            .src_blend = SLG_BLEND_SRC_ALPHA,
            .dest_blend = SLG_BLEND_INV_SRC_ALPHA,
            .blend_op = SLG_BLEND_OP_ADD,
            .src_blend_alpha = SLG_BLEND_ONE,
            .dest_blend_alpha = SLG_BLEND_INV_SRC_ALPHA,
            .blend_op_alpha = SLG_BLEND_OP_ADD,
            .color_write_enable =  {1, 1, 1, 1}
        },
        .format_overrides = format_override,
        .num_overrides = 3
        
    });
   
}

void slimgui_frame(){
    //slg_app_data_t* app_data = &slg_d3d12_state.appdata;
    io->MousePos = (ImVec2){(float)slimgui_input_state.mouse_x,(float)slimgui_input_state.mouse_y};
    io->MouseDown[0] = slimgui_input_state.left_mouse_down;
    io->MouseDown[1] = slimgui_input_state.right_mouse_down;
    io->MouseWheel = slimgui_input_state.mouse_scroll_delta / (float)WHEEL_DELTA;
    slimgui_input_state.mouse_scroll_delta = 0;
    igNewFrame();
    //igShowDemoWindow(NULL);

    
}

void slimgui_end_frame(){
    slg_framebuffer_t frame_buf = slg_d3d12_state.frame_buf;
    slg_desc_t desc = slg_d3d12_state.desc;
    igRender();
    ImDrawData* draw_data = igGetDrawData();
    
    ID3D12GraphicsCommandList* command_list = desc.commandList;   

    size_t total_verts = 0;
    size_t total_index = 0;
    for(int i = 0;i<draw_data->CmdListsCount;i++){
        ImDrawList* list = draw_data->CmdLists.Data[i];
        total_verts += list->VtxBuffer.Size;
        total_index += list->IdxBuffer.Size;
    }


    //COME BACK TO THIS. NOT SURE IF I WANT TO HAVE A DYNAMIC ALLOCATION EVERY FRAME LIKE THIS
    ImDrawVert* final_verts = malloc(total_verts * sizeof(ImDrawVert));
    ImDrawIdx* final_index = malloc(total_index * sizeof(ImDrawIdx));
    ImDrawVert* vert_ptr = final_verts;//going to be using this to index into the array
    ImDrawIdx* idx_ptr = final_index;
    for(int i = 0;i<draw_data->CmdListsCount;i++){
        
        ImDrawList* list = draw_data->CmdLists.Data[i];

        ImDrawVert* verts = list->VtxBuffer.Data;
        size_t current_vertex_size = list->VtxBuffer.Size;
       

        ImDrawIdx* index = list->IdxBuffer.Data;
        size_t current_index_size = list->IdxBuffer.Size;
        
        memcpy(vert_ptr, verts, current_vertex_size * sizeof(ImDrawVert));
        memcpy(idx_ptr, index, current_index_size * sizeof(ImDrawIdx));
        vert_ptr += current_vertex_size;
        idx_ptr += current_index_size;

       
        
    }
    //update current frame buffers with the corresponding information
    //command_list->lpVtbl->RSSetScissorRects(command_list,1,&scissor_rect);
    slg_update_buffer(slimgui_frame_buffer.index_buffers[frame_buf.frameIndex],(void*)final_index,(unsigned int)total_index * sizeof(ImDrawIdx));
    slg_update_buffer(slimgui_frame_buffer.vertex_buffers[frame_buf.frameIndex],(void*)final_verts,(unsigned int)total_verts * sizeof(ImDrawVert));
    slg_set_pipeline(&slimgui_pip);
    slg_set_bindings(&slimgui_frame_buffer.bind[frame_buf.frameIndex]);
    
    size_t current_index_draw = 0;
    size_t current_vertex_draw = 0;
   
    RECT rect;
    GetClientRect(app_data_d3d12.hwnd, &rect);
    for(int i = 0;i<draw_data->CmdListsCount;i++){
        ImDrawList* list = draw_data->CmdLists.Data[i];
        ImVec2 clip_off = draw_data->DisplayPos;
        for(int n  = 0;n<list->CmdBuffer.Size;n++){
            
            ImDrawCmd cmd = list->CmdBuffer.Data[n];
            bool changed_rt = false;
            //this is a dumb way to do this. I need to make the textures themselves hold onto the heap maybe
            //or have some other typing that I am passing in instead of the rendertexture. I am already creating textures to do things
            //I should just be able to pass any texture type. slg_texture,slg_render_texture,slg_depth_texture and let it be

            if(cmd.TextureId != io->Fonts->TexID){ //need some way to discern the typing. Either RT or TEX. OR JUST ONLY PASS RT's in
                //TAKE THE CURRENT BINDINGS AND BEND IT TO YOUR WILL. BEND IT TO YOUR WILL NOW
                slg_inject_texture((slg_texture*)cmd.TextureId);
                changed_rt = true;
            }

            D3D12_RECT scissor_rect;
            scissor_rect.left   = (LONG)(cmd.ClipRect.x - clip_off.x);
            scissor_rect.top    = (LONG)(cmd.ClipRect.y - clip_off.y);
            scissor_rect.right  = (LONG)(cmd.ClipRect.z - clip_off.x);
            scissor_rect.bottom = (LONG)(cmd.ClipRect.w - clip_off.y);
            command_list->lpVtbl->RSSetScissorRects(command_list,1,&scissor_rect);
            D3D12_VIEWPORT viewport = {
                0.0f, 0.0f,
                io->DisplaySize.x,
                io->DisplaySize.y,
                0.0f, 1.0f
            };
            command_list->lpVtbl->RSSetViewports(command_list,1,&viewport);
            //set the bindings and draw
            //this should be the proper draw call
            slg_draw((unsigned int)cmd.ElemCount,1,(unsigned int)cmd.IdxOffset + (unsigned int)current_index_draw,(unsigned int)current_vertex_draw,0);

            if(changed_rt){
                slg_set_bindings(&slimgui_frame_buffer.bind[frame_buf.frameIndex]);
            }
        }
        current_index_draw += list->IdxBuffer.Size;
        current_vertex_draw += list->VtxBuffer.Size;
        
    }
    free(final_verts);
    free(final_index);
}




const unsigned char imgui_ps_cso[] = {
  
};
const unsigned int imgui_ps_cso_size = 4025;

const unsigned char imgui_vs_cso[] = {
  
};
const unsigned int imgui_vs_cso_size = 4355;
#endif 


#endif